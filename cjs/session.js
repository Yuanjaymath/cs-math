...
class Session{
 constructor(level=1,goal=5){this.level=level;this.goal=goal;this.maxHp=50;this.hp=this.maxHp;this.shield=0;this.done=0;this.streak=0;this.bestStreak=0;this.correctFirst=0;this.records=[];this.state='playing';this.question=null;this.attempts=new Set();this.solved=false;this.score=0;}
 ...
 answer(value,seconds){if(this.state!=='playing'||this.solved||!this.question||this.attempts.has(value)||!this.question.options.includes(value))return {type:'ignored'};
  ...
  this.solved=true;this.done++;const first=this.attempts.size===0;if(first){this.correctFirst++;this.streak++;}this.bestStreak=Math.max(this.bestStreak,this.streak);this.hp=Math.min(this.maxHp,this.hp+8);if(this.streak>0&&this.streak%3===0)this.shield=Math.min(50,this.shield+25);this.score+=100+(first?50:0)+Math.min(this.streak,5)*10;
